...
import { Service, ServiceAddon, AppSubscriptionPlan, BillingInterval } from '../../types';
import { AIBioModal } from '../ai/AIBioModal';
import { BarberAvailabilityCalendar } from './BarberAvailabilityCalendar';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20 md:pb-12" id="barber-profile-manage-view">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-white font-serif">Barber Profile & Service Hub</h1>
          <p className="text-xs text-slate-400">Manage client-facing biography, menu pricing, licenses, and availability calendar</p>
        </div>
        {activeTab === 'profile' && (
          <button
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-2">
        ...
        >
          ...
          <span>Bio, License & Services Catalog</span>
        </button>
        ...
        >
          ...
          <span>Availability Calendar & Blocked Dates</span>
        </button>
      </div>

      {/* Barber Pro Tier Membership Summary */}
      <div className="rounded-3xl border border-amber-500/30 bg-gradient-to-r from-amber-500/10 via-slate-900 to-slate-900 p-5 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-500 text-slate-950 shadow-md shadow-amber-500/20 font-black">
            <Crown className="h-6 w-6" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-bold text-white">
                {plan?.name || 'Free Barber'} Membership
              </h3>
              <span className="rounded-full bg-amber-500/20 border border-amber-400/30 text-amber-300 px-2.5 py-0.5 text-[10px] font-black uppercase">
                {subscription?.status || 'Active'}
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">
              {plan?.id === 'pro' || plan?.id === 'premium'
                ? '0% Commission Fee • Radius Boost & Priority Matching Active'
                : 'Upgrade to Barber Pro for 0% commission fees, 25-mile radius boost, and verified pro badge.'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-amber-500 px-4 py-2 text-xs font-bold text-slate-950 shadow-sm hover:bg-amber-400 transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-xs font-bold text-white hover:bg-slate-700 transition-colors"
          >
      ...
      {activeTab === 'availability' ? (
        <BarberAvailabilityCalendar barberId={user?.id} />
      ) : (
